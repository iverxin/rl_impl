...
ENV_ID = 'Pendulum-v0' #'BipedalWalker-v2'  # BipedalWalkerHardcore-v2   BipedalWalker-v2  LunarLanderContinuous-v2
...
MAX_GLOBAL_EP = 200  # number of training episodes
    ...
    def __init__(self, scope):
        ...
        def get_actor(input_shape):  # policy network
            with tf.name_scope(self.scope):
                ...
                nn = tl.layers.Dense(n_units=100, act=tf.nn.relu6, W_init=w_init, name='la')(ni)
                nn = tl.layers.Dense(n_units=100, act=tf.nn.relu6, W_init=w_init, name='la2')(nn)
        ...
        def get_critic(input_shape):  # we use Value-function here, but not Q-function.
            with tf.name_scope(self.scope):
                ni = tl.layers.Input(input_shape, name='in')
                nn = tl.layers.Dense(n_units=100, act=tf.nn.relu6, W_init=w_init, name='lc')(ni)
                nn = tl.layers.Dense(n_units=100, act=tf.nn.relu6, W_init=w_init, name='lc2')(nn)
