...
def create_actor_network(input_state_shape, action_dim, action_bound, name=""):
    """
    create a policy net
    :param self:
    ...
    model = keras.models.Model(inputs=input, outputs=[mu, sigma], name=name)
    print(model.summary())
...
def create_critic_network(input_state_shape, input_action_shape, name=''):
    """
    ...
    model = keras.models.Model(inputs=status_input, outputs=out, name='Critic ' + name)
